...
def exec_stmt(
    ...
    *,
    ...
    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
        # Analyze function bodies as security-relevant code slices. Function
        # parameters are initialized as neutral values so framework objects such
        # as request/cursor are not reported as vulnerabilities by themselves.
        func_lab = lab.copy()
        ...
        for arg in args:
            func_lab.set(arg.arg, MultiLabel(policy))
        _ = exec_stmt_list(node.body, policy, func_lab, vulns, pc, while_unroll=while_unroll)
        return lab
...
def scan_code(code: str, patterns_path: str) -> List[dict]:
    """Analyze Python source code and return vulnerability records."""
    ...
    return vulns.records()


def scan_file(slice_path: str, patterns_path: str) -> List[dict]:
    """Analyze a Python file and return vulnerability records."""
    with open(slice_path, "r", encoding="utf-8") as f:
        code = f.read()
    return scan_code(code, patterns_path)
...
def main(argv: List[str]) -> int:
    ...
    records = scan_file(slice_path, patterns_path)
